"""OpenAI-compatible client kwargs — base_url and SSL for self-hosted / self-signed certs."""
import httpx
...
def get_openai_client_kwargs(settings: Settings) -> dict:
    """
    Build kwargs for llama_index OpenAI/OpenAIEmbedding when using
    self-hosted or OpenAI-compatible APIs with optional self-signed TLS.
    ...
    - OPENAI_SSL_VERIFY: set False only if no CA bundle available (insecure).
    """
    kwargs: dict = {}
    ...
    if verify is not True:
        ...
        kwargs["async_http_client"] = httpx.AsyncClient(verify=verify)
